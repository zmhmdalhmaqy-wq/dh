# app.py - النسخة النهائية مع دعم كامل لـ Python و Node.js
import os
import json
import re
import subprocess
...
app.secret_key = "ZIAD_HOST_SECURE_KEY_2026"
...
def check_nodejs():
    """التحقق من وجود Node.js على النظام"""
    ...
    for node_cmd in node_paths:
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"✅ تم العثور على Node.js: {version}")
                return node_cmd, version
        except:
            continue
    print("⚠️ Node.js غير موجود على النظام، سيتم دعم Python فقط")
    return None, None

NODEJS_CMD, NODEJS_VERSION = check_nodejs()
...
def start_server_process(folder):
    ...
    try:
        ...
        if server_type == "Node.js":
            # التحقق من وجود Node.js
            if not NODEJS_CMD:
                log_file.write("❌ Node.js غير مثبت على النظام!\n")
                log_file.write("💡 يرجى تثبيت Node.js أو استخدام Python فقط.\n")
                log_file.close()
                return False, "Node.js غير مثبت على الخادم"
            
            cmd = [NODEJS_CMD, main_file]
            proc = subprocess.Popen(
                cmd,
                cwd=srv["path"],
                stdout=log_file,
        ...
        err = f"❌ {'Node.js' if server_type == 'Node.js' else 'Python'} غير موجود: {str(e)}"
        log_file.write(err + "\n")
        log_file.close()
        return False, err
    except Exception as e:
...
def api_register():
    ...
    threading.Thread(
        target=notify_admin,
        args=(
            f"🔔 *مستخدم جديد اشترك في ZIAD HOST!*\n"
            f"👤 المستخدم: `{username}`\n"
...
@app.route('/api/nodejs/status')
def nodejs_status():
    """التحقق من حالة Node.js"""
    if NODEJS_CMD:
        return jsonify({"installed": True, "version": NODEJS_VERSION, "path": NODEJS_CMD})
    return jsonify({"installed": False, "message": "Node.js غير مثبت"})
...
def add_server():
    ...
    # التحقق من وجود Node.js إذا كان المستخدم يريد Node.js
    if server_type == "Node.js" and not NODEJS_CMD:
        return jsonify({"success": False, "message": "⚠️ Node.js غير مثبت على الخادم حالياً. يرجى استخدام Python أو التواصل مع الدعم."})
...
def bot_create_server():
    ...
    # التحقق من وجود Node.js
    if server_type == "Node.js" and not NODEJS_CMD:
        return jsonify({"success": False, "message": "⚠️ Node.js غير مثبت على الخادم"}), 400
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("🚀 ZIAD HOST - نظام الاستضافة المتكامل")
    print("="*60)
    print(f"🐍 Python: {sys.version}")
    if NODEJS_CMD:
        print(f"🟨 Node.js: {NODEJS_VERSION} ({NODEJS_CMD})")
    else:
        print("⚠️ Node.js: غير مثبت - سيتم دعم Python فقط")
    print(f"🌐 المنفذ: {port}")
    print("="*60 + "\n")
